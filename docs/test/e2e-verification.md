...
AI Client (MCP SSE)
       |
   center :18080 (HTTP/MCP)
       |      |
       |   :19090 (gRPC)
       |      |
   agent-1    proxy-1 :19091 (gRPC)
(test-agent-1)    |
            agent-2
          (test-agent-2)
```
...
| 组件         | 监听地址      | 连接上游          | 主机名          |
|--------------|--------------|-------------------|----------------|
| center       | :18080 (HTTP), :19090 (gRPC) | PostgreSQL :5432 | — |
| agent-1      | —            | center :19090     | test-agent-1   |
| proxy-1      | :19091 (gRPC) | center :19090    | test-proxy-1   |
| agent-2      | —            | proxy-1 :19091    | test-agent-2   |
