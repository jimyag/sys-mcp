...
sysplane --server http://center.example.com:18880 --token your-client-token commands invoke fs.read --node <node-id> --params '{"path":"/etc/hostname"}'
sysplane --server http://center.example.com:18880 --token your-admin-token templates list
sysplane --server http://center.example.com:18880 --token your-admin-token commands invoke echo.hello --nodes <node-id> --params '{}'
...
- `commands invoke` 是统一执行入口：builtin 和 command template 都从这里走。
- `commands invoke` 直接传模板名即可；CLI 会先解析到模板 ID 再调用统一的 `/v1/invocations`。
- `commands invoke` 统一通过 `--params` / `--params-file` 传参数，不再为单个 builtin 保留特殊参数入口。
- `commands invoke` 始终输出统一的 Invocation JSON 响应，不再对 `fs.read` 之类动作做特殊格式化。
